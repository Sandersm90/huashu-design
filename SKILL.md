...
## Local Preview Workflow (v1)

When a task needs **show-while-building, choose-in-browser review** — for example design-direction selection, prototype flow review, Tweaks exploration, or browser-side feedback — you can use the local preview server. It is not Vite and not a full frontend dev server. It is a **lightweight review runtime for HTML deliverables**.

### When to use it

Prefer preview mode when:
- the user needs to see the current design in the browser instead of only screenshots
- the task benefits from A/B/C direction picking
- the user should click through a prototype flow while work is still in progress
- the agent needs to push multiple screens over time while the user reacts in-browser

Do **not** use preview mode when:
- a single delivered HTML file is enough
- the task is only export-oriented (MP4 / GIF / PDF / PPTX) with no review loop
- the user only needs textual advice, not a visual surface
...
- `url` — the browser URL to open
- `content_dir` — where to write preview HTML
- `state_dir` — where to read `events` / `server-info`
...
If you just want to verify the preview system itself, load the built-in demos:
...
2. Write a new `.html` file into `content_dir`
3. The browser shows the **newest** HTML file and auto-reloads on update
4. Clicks on elements with `data-choice` are written to `state_dir/events`
5. On the next turn, combine the user's terminal response with those events and continue the design loop

### Two output modes

**Full HTML documents**: if you already built a complete prototype, demo, or review screen, write full HTML. The server renders it as-is and only injects the preview helper.

**Lightweight fragments**: if you only need to show 2–3 directions, layouts, or style choices, you do not need a full document. Write only the core body content and the server will wrap it in the preview frame.

### Interaction conventions

- Put `data-choice="..."` on any selectable item
- Use `data-multiselect` on multi-select containers when needed
- If you need to send state explicitly from browser code, use:

```js
window.huashuPreview.select('takram')
window.huashuPreview.state('palette', 'warm')
window.huashuPreview.note('user paused on option b')
```

### Delivery discipline

- Do **not** keep overwriting the same filename forever — push new screens as the review evolves so history stays understandable
- Preview is for review and iteration; final deliverables are still HTML / MP4 / GIF / PDF / PPTX depending on the task
- If the preview references local assets, place them alongside the relevant content inside `content_dir` and use relative paths

### Example prompts

These prompts should naturally trigger preview mode:
