...
当任务需要**边做边看、边看边选**（如设计方向三选一、prototype flow review、Tweaks 变体切换、用户在浏览器里点选反馈）时，可以启用本地 preview server。它不是 Vite，也不是完整 frontend dev server；它是一个**给 HTML 产物用的轻量 review runtime**。

### 什么时候该用

优先用于这些场景：
- 用户需要在浏览器里直接看当前方案，而不是只看截图
- 需要 A/B/C 方案点选
- 需要在浏览器里试点原型流程
- 需要 agent 多次推送新 screen，让用户一边看一边反馈

不必用 preview 的场景：
- 一次性交付单个 HTML 文件，双击即可看
- 只是导出 MP4 / GIF / PDF / PPTX，不需要交互 review
- 用户只要文字建议，不需要视觉界面
...
- `state_dir`：从这里读 `events` / `server-info`
...
1. 启动 preview server
2. 往 `content_dir` 写一个新的 `.html` 文件
3. 浏览器会自动显示**最新**的 HTML，并在更新后自动 reload
4. 用户点击带 `data-choice` 的元素时，事件会记到 `state_dir/events`
5. 下一轮根据用户终端回复 + `events` 一起继续设计

### 两种写法

**完整 HTML 文档**：如果你已经做出了完整 prototype / demo / slide review screen，就直接写完整 HTML。server 会原样渲染，只额外注入 preview helper。

**轻量 fragment**：如果你只是想快速给用户看 2-3 个方向 / layout / style choice，不必每次都写完整 HTML。只写 body 内的核心内容，server 会自动包一层 preview frame。

### 交互约定

- 任何可选项都尽量加 `data-choice="xxx"`
- 多选容器可加 `data-multiselect`
- 需要主动发状态时，可在浏览器里调用：
